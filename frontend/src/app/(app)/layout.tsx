...
export default function AppLayout({ children }: { children: ReactNode }) {
  ...
  const renderLogo = (isCollapsed: boolean) => (
    ...
    >
      ...
      {!isCollapsed && (
        <span className="text-sm font-semibold tracking-tight text-zinc-100">MigratorGen</span>
      )}
    </Link>
  );

  const renderUserSection = (isCollapsed: boolean) => (
    <div className={`border-t border-white/5 px-3 py-3 ${isCollapsed ? "flex flex-col items-center gap-2" : ""}`}>
      {mounted && session?.user ? (
        <div className={`flex items-center ${isCollapsed ? "justify-center" : "gap-3 px-1"} py-1`}>
          <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-zinc-700 text-[11px] font-semibold text-zinc-300">
            {avatarLetter}
          </div>
          {!isCollapsed && (
            <span className="min-w-0 truncate text-xs text-zinc-500">{email}</span>
          )}
        </div>
      ) : (
        <div className={`flex items-center ${isCollapsed ? "justify-center" : "gap-3 px-1"} py-1`}>
      ...
      <button
        ...
        title="Sign out"
        className={`btn-press flex w-full items-center rounded-xl text-sm font-medium text-zinc-500 transition-colors hover:bg-red-500/10 hover:text-red-400 ${
          isCollapsed ? "justify-center px-0 py-2.5" : "gap-3 px-3 py-2.5"
        }`}
      >
        <svg
  ...
  const renderCollapseToggle = () => (
    <button
      ...
      className="btn-press mb-3 flex w-full items-center justify-center border-t border-white/5 pt-3 text-zinc-600 transition-colors hover:text-blue-400"
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
        fill="none"
  ...
  return (
    <div className="flex h-screen bg-[#09090b]">
      ...
      <aside
        ...
        className="hidden shrink-0 flex-col border-r border-white/10 bg-[#18181b] transition-all duration-200 ease-in-out lg:flex"
      >
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          />
          <aside className="animate-slide-in absolute left-0 top-0 flex h-full w-[260px] flex-col border-r border-white/10 bg-[#18181b] shadow-xl">
            <button
              onClick={() => setMobileOpen(false)}
              aria-label="Close menu"
              className="absolute right-3 top-4 btn-press rounded-xl p-2 text-zinc-500 hover:bg-white/5 hover:text-zinc-300"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        {/* Header bar */}
        <header className="sticky top-0 z-40 flex h-[52px] shrink-0 items-center gap-2 border-b border-white/5 bg-[#09090b]/80 px-4 backdrop-blur-md lg:px-6">
          {/* Mobile hamburger */}
          <button
            onClick={() => setMobileOpen(true)}
            aria-label="Open menu"
            className="btn-press rounded-xl p-2 text-zinc-500 hover:bg-white/5 lg:hidden"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              title="Back"
              aria-label="Go back"
              className="btn-press rounded-xl p-2 text-zinc-500 hover:bg-white/5 disabled:pointer-events-none disabled:opacity-30"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                ...
                return (
                  <span key={crumb.href} className="flex min-w-0 items-center gap-1.5">
                    ...
                    {isLast ? (
                      ...
                      <Link
                        href={crumb.href}
                        className="truncate text-zinc-600 transition-colors hover:text-blue-400"
                      >
                        {crumb.label}
                      </Link>
                    )}
