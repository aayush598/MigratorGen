...
class SyncMigrationClient:
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._initialise()
        log.info("SyncMigrationClient initialised (mode=%s, base_url=%s)", self._client_mode, self._config.base_url)
    ...
    def migrate_code(self, source_code: str, rules: list[m.Rule], source_version: str = "1.0.0", target_version: str = "latest", dry_run: bool = False) -> m.MigrateResponse:
        return self._engine.migrate_code(source_code, rules, source_version=source_version, target_version=target_version, dry_run=dry_run)
    ...
    def preview_migration(self, source_code: str, rules: list[m.Rule], source_version: str = "1.0.0", target_version: str = "latest") -> m.DiffPreview:
        return self._engine.preview_migration(source_code, rules, source_version=source_version, target_version=target_version)
    ...
    def migrate_file(self, file_path: str, rules: list[m.Rule], source_version: str = "1.0.0", target_version: str = "latest", dry_run: bool = False) -> m.MigrateResponse:
        return self._engine.migrate_file(file_path, rules, source_version=source_version, target_version=target_version, dry_run=dry_run)
