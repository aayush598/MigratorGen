...
class AsyncRemoteService:
    def __init__(self, config: SDKConfig) -> None:
        ...
        self._client = httpx.AsyncClient(
            base_url=self._base_url, headers=headers, timeout=httpx.Timeout(config.timeout), limits=limits,
        )
    ...
    async def _request(self, method: str, path: str, params: dict[str, Any] | None = None, json_body: dict[str, Any] | None = None) -> Any:
        ...
        async def _do() -> Any:
            try:
                ...
                raise APIError(f"HTTP request failed: {exc}", details={"method": method, "path": path}) from exc
    ...
    async def migrate_code(self, source_code: str, rules: list[m.Rule], source_version: str = "1.0.0", target_version: str = "latest", dry_run: bool = False) -> m.MigrateResponse:
        payload = {"source_code": source_code, "rules": [r.to_dict() for r in rules], "source_version": source_version, "target_version": target_version, "dry_run": dry_run}
        data = await self._request("POST", "/api/v1/migrate", json_body=payload)
        return m.MigrateResponse(**data)

    async def preview_migration(self, source_code: str, rules: list[m.Rule], source_version: str = "1.0.0", target_version: str = "latest") -> m.DiffPreview:
        return await self.migrate_code(source_code, rules, source_version=source_version, target_version=target_version, dry_run=True)
    ...
    async def suggest_migrations(self, file_path: str, destination_library: str) -> Any:
        return await self._request("POST", "/api/v1/suggest", json_body={"path": file_path, "library": destination_library})
    ...
    async def generate_rules_from_diff(self, old_code: str, new_code: str, module: str = "") -> list[m.Rule]:
        data = await self._request("POST", "/api/v1/generate-rules/diff", json_body={"old_code": old_code, "new_code": new_code, "module": module})
        return [m.Rule(**r) for r in data.get("rules", [])]

    async def generate_rules_from_changelog(self, changelog_text: str, library_name: str = "unknown") -> m.VersionChangelog:
        data = await self._request("POST", "/api/v1/generate-rules/changelog", json_body={"changelog_text": changelog_text, "library_name": library_name})
        return m.VersionChangelog(**data)

    async def resolve_path(self, source_version: str, target_version: str, library_name: str) -> m.ResolvedPath:
        data = await self._request("POST", "/api/v1/resolve-path", json_body={"source_version": source_version, "target_version": target_version, "library_name": library_name})
        steps_data = data.pop("steps", [])
        path = m.ResolvedPath(**data)
        path.steps = [m.MigrationStep(**s) for s in steps_data]
        return path

    async def migrate_file(self, file_path: str, rules: list[m.Rule], source_version: str = "1.0.0", target_version: str = "latest", dry_run: bool = False) -> m.MigrateResponse:
        source_code = read_file(file_path)
        return await self.migrate_code(source_code, rules, source_version=source_version, target_version=target_version, dry_run=dry_run)

    async def generate_migrator_package(self, library_name: str, output_dir: str = ".") -> str:
        data = await self._request("POST", "/api/v1/generate-package", json_body={"library": library_name, "output_dir": output_dir})
...
class SyncRemoteService:
    def __init__(self, config: SDKConfig) -> None:
        ...
        if config.api_key:
            headers["Authorization"] = f"Bearer {config.api_key}"
        limits = httpx.Limits(max_keepalive_connections=20, max_connections=100, keepalive_expiry=30)
        self._client = httpx.Client(base_url=self._base_url, headers=headers, timeout=httpx.Timeout(config.timeout), limits=limits)
        self._retry = RetryStrategy(max_retries=config.max_retries)
    ...
    def migrate_code(self, source_code: str, rules: list[m.Rule], source_version: str = "1.0.0", target_version: str = "latest", dry_run: bool = False) -> m.MigrateResponse:
        payload = {"source_code": source_code, "rules": [r.to_dict() for r in rules], "source_version": source_version, "target_version": target_version, "dry_run": dry_run}
        data = self._request("POST", "/api/v1/migrate", json_body=payload)
        return m.MigrateResponse(**data)

    def preview_migration(self, source_code: str, rules: list[m.Rule], source_version: str = "1.0.0", target_version: str = "latest") -> m.DiffPreview:
        return self.migrate_code(source_code, rules, source_version=source_version, target_version=target_version, dry_run=True)
    ...
    def suggest_migrations(self, file_path: str, destination_library: str) -> Any:
        return self._request("POST", "/api/v1/suggest", json_body={"path": file_path, "library": destination_library})
    ...
    def generate_rules_from_diff(self, old_code: str, new_code: str, module: str = "") -> list[m.Rule]:
        data = self._request("POST", "/api/v1/generate-rules/diff", json_body={"old_code": old_code, "new_code": new_code, "module": module})
        return [m.Rule(**r) for r in data.get("rules", [])]

    def generate_rules_from_changelog(self, changelog_text: str, library_name: str = "unknown") -> m.VersionChangelog:
        data = self._request("POST", "/api/v1/generate-rules/changelog", json_body={"changelog_text": changelog_text, "library_name": library_name})
        return m.VersionChangelog(**data)

    def resolve_path(self, source_version: str, target_version: str, library_name: str) -> m.ResolvedPath:
        data = self._request("POST", "/api/v1/resolve-path", json_body={"source_version": source_version, "target_version": target_version, "library_name": library_name})
        steps_data = data.pop("steps", [])
        path = m.ResolvedPath(**data)
        path.steps = [m.MigrationStep(**s) for s in steps_data]
        return path

    def migrate_file(self, file_path: str, rules: list[m.Rule], source_version: str = "1.0.0", target_version: str = "latest", dry_run: bool = False) -> m.MigrateResponse:
        source_code = read_file(file_path)
        return self.migrate_code(source_code, rules, source_version=source_version, target_version=target_version, dry_run=dry_run)

    def generate_migrator_package(self, library_name: str, output_dir: str = ".") -> str:
        data = self._request("POST", "/api/v1/generate-package", json_body={"library": library_name, "output_dir": output_dir})
        return data.get("path", "")
