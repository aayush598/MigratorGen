  ...
  return (
    <div className="max-w-4xl mx-auto animate-fade-up">
      <div className="mb-8">
        ...
        <p className="text-[14px] text-slate-500 mt-1">Choose the plan that fits your needs</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-10">
        {PLANS.map((plan) => (
          <div key={plan.name} className={`bg-white rounded-xl border p-6 flex flex-col transition-all ${
            plan.highlight ? "border-blue-500/30 shadow-lg shadow-blue-500/5" : "border-slate-200 hover:border-white/20"
          }`}>
            {plan.highlight && (
            ...
            <h3 className="text-[16px] font-semibold text-slate-900">{plan.name}</h3>
            <div className="mt-3 mb-5">
              <span className="text-[32px] font-bold text-slate-900 tracking-tight">{plan.price}</span>
              {plan.period && <span className="text-[13px] text-slate-400 ml-1">{plan.period}</span>}
            </div>
            ...
            {plan.current ? (
              <div className="text-center py-3 rounded-lg bg-white/5 text-[13px] font-semibold text-slate-400 border border-slate-200">Current plan</div>
            ) : plan.highlight ? (
              ...
              <button className="text-center py-3 rounded-lg border border-slate-200 text-[13px] font-semibold text-slate-600 hover:bg-white/5 transition-colors btn-press">
                Contact sales
              </button>
            )}
          </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6">
        <h2 className="text-[15px] font-semibold text-slate-900 mb-5">Frequently asked questions</h2>
        <div className="space-y-4">
          {[
            ...
            <div key={faq.q} className="p-4 rounded-lg bg-white/[0.02] border border-white/5">
              <p className="text-[13px] font-semibold text-slate-900">{faq.q}</p>
              <p className="text-[13px] text-slate-500 mt-1">{faq.a}</p>
            </div>
          ))}
        </div>
      </div>
